...
#pragma warning( disable : 4245 ) // conversion from 'int' to 'uint32_t', signed/unsigned mismatch
#pragma warning( disable : 4389 ) // signed/unsigned mismatch



#define MP_NEW(T) new (malloc(sizeof(T))) T
#define MP_DELETE(p) DeleteHelper(p)

template<typename T> void DeleteHelper(T* p)
{
	if (p)
	{
		p->~T();
...
void MicroProfileInit()
{
	...
	if(bOnce)
	{
		...
		S.nOpacityBackground = uint32_t(0xff<<24);
		S.nOpacityForeground = uint32_t(0xff<<24);
...
void MicroProfileShutdown()
{
	...
	if(S.pContextSwitchThread)
	{
		...
		MP_DELETE(S.pContextSwitchThread);
	}
#endif
...
MicroProfileThreadLog* MicroProfileCreateThreadLog(const char* pName)
{
	...
	{
		pLog = MP_NEW(MicroProfileThreadLog);
...
void MicroProfileStartContextSwitchTrace()
{
	...
	{
		if(!S.pContextSwitchThread)
			S.pContextSwitchThread = MP_NEW(std::thread)();
		if(S.pContextSwitchThread->joinable())
		{
			S.bContextSwitchStop = true;
			S.pContextSwitchThread->join();

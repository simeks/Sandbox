...
#include <Foundation/Container/StringUtil.h>
...
//-------------------------------------------------------------------------------
template<typename T, typename U>
INLINE bool sb::test_assert_helper::AssertEqual(const char* file, int line, T value, U expected,
	TestResult& test_result)
{
	if ((U)value != expected)
	{
		char value_str[1024];
		string_util::ToString(value_str, 1024, value);

		char expected_str[1024];
		string_util::ToString(expected_str, 1024, expected);

		FormatAssertMessage(test_result, file, line, "equality failed: %s != %s", value_str, expected_str);
...
template<typename T, typename U>
INLINE bool sb::test_assert_helper::AssertNotEqual(const char* file, int line, T value, U expected,
	TestResult& test_result)
{
	if ((U)value == expected)
	{
		...
		FormatAssertMessage(test_result, file, line, "inequality failed: %s == %s", value_str, expected_str);
...
template<>
INLINE bool sb::test_assert_helper::AssertEqual(const char* file, int line, const char* value,
...
template<>
INLINE bool sb::test_assert_helper::AssertNotEqual(const char* file, int line, const char* value,
	const char* expected, TestResult& test_result)
{
	if (strcmp(value, expected) == 0)
	{
...
INLINE bool sb::test_assert_helper::AssertEqualFloat(const char* file, int line, double value,
...
INLINE bool sb::test_assert_helper::AssertNotEqualFloat(const char* file, int line, double value,
	double expected, double tolerance, TestResult& test_result)
{
	if (fabs(value - expected) <= tolerance)
	{
...
INLINE bool sb::test_assert_helper::AssertExpression(const char* file, int line, const char* expr, bool expr_result,
	TestResult& test_result)
{
	if (!expr_result)
	{
...
class TEST_CLASS_NAME(test_case_name) : protected sb::TestCase \
{ \
public: \
	TEST_CLASS_NAME(test_case_name)() : TestCase(#test_case_name) { \
	sb::test_framework::RegisterTest(this); \
	} \
	virtual void Run(sb::TestResult& _test_result); \
}; \
	static TEST_CLASS_NAME(test_case_name) s_test_##test_case_name; \
	void TEST_CLASS_NAME(test_case_name)::Run(sb::TestResult& _test_result) \
...
{ if (!sb::test_assert_helper::AssertExpression(__FILE__, __LINE__, #expression, expression, _test_result)) \
{ _test_result.result = sb::TestResult::FAILED; return; } } \

#define ASSERT_EQUAL(value, expected) \
{ if (!sb::test_assert_helper::AssertEqual(__FILE__, __LINE__, value, expected, _test_result)) \
{ _test_result.result = sb::TestResult::FAILED; return; } } \

#define ASSERT_NOT_EQUAL(value, expected) \
{ if (!sb::test_assert_helper::AssertNotEqual(__FILE__, __LINE__, value, expected, _test_result)) \
{ _test_result.result = sb::TestResult::FAILED; return; } }

#define ASSERT_EQUAL_F(value, expected, tolerance) \
{ if (!sb::test_assert_helper::AssertEqualFloat(__FILE__, __LINE__, value, expected, tolerance, _test_result)) \
{ _test_result.result = sb::TestResult::FAILED; return; } }

#define ASSERT_NOT_EQUAL_F(value, expected, tolerance) \
{ if (!sb::test_assert_helper::AssertNotEqualFloat(__FILE__, __LINE__, value, expected, tolerance, _test_result)) \
{ _test_result.result = sb::TestResult::FAILED; return; } }

#define ASSERT_EQUAL_STR(value, expected) \
{ if (!sb::test_assert_helper::AssertEqual<const char*, const char*>(__FILE__, __LINE__, value, expected, _test_result)) \
{ _test_result.result = sb::TestResult::FAILED; return; } } \

#define ASSERT_NOT_EQUAL_STR(value, expected) \
{ if (!sb::test_assert_helper::AssertNotEqual<const char*, const char*>(__FILE__, __LINE__, value, expected, _test_result)) \
{ _test_result.result = sb::TestResult::FAILED; return; } }
